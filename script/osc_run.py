...
from run_utils import LEFT_STANCE, RIGHT_STANCE, FLIGHT_AFTER_LEFT, FLIGHT_AFTER_RIGHT, get_fsm, IMPACT_AFTER_LEFT, \
    IMPACT_AFTER_RIGHT
...
class OperationalSpaceRunningController(LeafSystem):
    def __init__(self, gains: OscGains):
        ...
        self.contact_points = {
            ...
            IMPACT_AFTER_LEFT:PointOnFrame(
                self.plant.GetBodyByName("right_lower_leg").body_frame(),
                np.array([0, 0, -0.5])
            ),
            IMPACT_AFTER_RIGHT:PointOnFrame(self.plant.GetBodyByName("left_lower_leg").body_frame(),
                np.array([0, 0, -0.5])
            )
        }
        self.swing_foot_points = {
            ...
            FLIGHT_AFTER_LEFT: self.contact_points[RIGHT_STANCE],
            FLIGHT_AFTER_RIGHT: self.contact_points[LEFT_STANCE],
            IMPACT_AFTER_LEFT: self.contact_points[IMPACT_AFTER_LEFT],
            IMPACT_AFTER_RIGHT: self.contact_points[IMPACT_AFTER_RIGHT]
        }
    ...
    def SetupAndSolveQP(self, context: Context) -> Tuple[np.ndarray, MathematicalProgram]:
        ...
        if fsm in [LEFT_STANCE, RIGHT_STANCE,IMPACT_AFTER_LEFT,IMPACT_AFTER_RIGHT]:
        ...
        # liftoff_position = planner.GetSwingFootLiftoffPosition()
        ...
        #prog.AddLinearConstraint(u>np.ones_like(u)*-10)
        #prog.AddLinearConstraint(u<np.ones_like(u)*10)
        # Dynamics constraint
        if fsm in [LEFT_STANCE, RIGHT_STANCE,IMPACT_AFTER_LEFT,IMPACT_AFTER_RIGHT]:
